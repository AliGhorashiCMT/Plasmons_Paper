...
μ = dirac_point + 0.5;
subsampling_gaussian = returnfermikpoint(Hwannier, cell_map, dirac_point+0.5, Val(2), mesh=30, num_blocks=1000, 
    weight=Val(:gaussian), histogram_width=10, esmearing=0.01)

tau_gaussian =  τ(Hwannier, cell_map, Pwannier, force_matrix, cellph_map,
    Heph, celleph_map, collect(0.01:0.01:1), dirac_point+0.5, Val(:gaussian), Val(2); 
    esmearing=0.01, supplysampling=subsampling_gaussian, supplydos = 0.028, histogram_width=10, intraband=5, mesh=10, num_blocks=100, fracroom=1)
