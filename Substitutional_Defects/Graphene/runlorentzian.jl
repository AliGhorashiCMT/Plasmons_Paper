...
μ = dirac_point + 0.5;
subsampling_lorentzian = returnfermikpoint(Hwannier, cell_map, dirac_point+0.5, Val(2), mesh=30, num_blocks=3000, 
    weight=Val(:lorentzian), histogram_width=10, esmearing=esmearing, cutoff=1e-4)

tau_lorentzian =  τ(Hwannier, cell_map, Pwannier, force_matrix, cellph_map,
    Heph, celleph_map, collect(0.01:0.01:1), dirac_point+0.5, Val(:lorentzian), Val(2); 
    esmearing=esmearing, supplysampling=subsampling_lorentzian, supplydos = 0.028, histogram_width=10, intraband=5, mesh=10, num_blocks=100, fracroom=1)
